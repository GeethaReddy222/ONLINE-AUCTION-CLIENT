...
export default function CustomerProfile() {
  ...
  useEffect(() => {
    ...
    axios
      .get('http://localhost:4000/customer/profile', {
        headers: {
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center">
              ...
              <h2 className="mt-4 text-2xl font-bold text-blue-600">{customer?.username}</h2>
              <p className="mt-2 text-gray-600">Email: {customer?.email}</p>
              <p className="mt-2 text-gray-600">Contact: {customer?.contactNumber}</p>
              <p className="mt-2 text-gray-600 text-center max-w-md">Address: {customer?.address}</p>
            </div>
          )}
        </main>
      </div>
